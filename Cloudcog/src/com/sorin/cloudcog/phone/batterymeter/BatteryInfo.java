...
class BatteryInfo {
	...
	public int percent, status, health, plugged, temperature, voltage,
			last_status, last_plugged, last_percent;
	...
	public static class RelativeTime {
		...
		// If days > 0, then minutes is undefined and hours is rounded to the
		// closest hour (rounding minutes up or down)
	...
	public void load(SharedPreferences sp_store) {
		...
		last_status_cTM = sp_store.getLong(KEY_LAST_STATUS_CTM,
				DEFAULT_LAST_STATUS_CTM);
	...
	private static int attemptOnePercentHack(int percent) {
		java.io.File hack_file = new java.io.File(
		...
		if (hack_file.exists()) {
			try {
				...
				if (charge_counter < percent + 10
						&& charge_counter > percent - 10) {
					...
					/*
					 * The Log messages are only really useful to me and might
					 * as well be left hardwired here in English.
					 */
					Log.e(LOG_TAG, "charge_counter file exists but with value "
							+ charge_counter
				...
				Log.e(LOG_TAG,
						"Read charge_counter file but couldn't convert contents to int");
				e.printStackTrace();
			}
		}
