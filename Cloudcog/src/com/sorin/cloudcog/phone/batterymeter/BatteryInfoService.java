...
public class BatteryInfoService extends Service {
	private final IntentFilter batteryChanged = new IntentFilter(
			Intent.ACTION_BATTERY_CHANGED);
	private final IntentFilter userPresent = new IntentFilter(
	...
	private final Runnable mPluginNotify = new Runnable() {
		public void run() {
			try {
				...
				java.lang.reflect.Method m = c.getMethod("notify", new Class[] {
						int.class, int.class, String.class, String.class,
						PendingIntent.class });
				m.invoke(pluginServiceConnection.service, new Object[] {
						info.percent, info.status, mainNotificationTopLine,
						mainNotificationBottomLine, mainWindowPendingIntent });
	...
	public void onCreate() {
		...
		mVibrator = (android.os.Vibrator) context
		...
		mainWindowPendingIntent = PendingIntent.getActivity(context, 0,
				mainWindowIntent, 0);

		Intent updatePredictorIntent = new Intent(context,
				BatteryInfoService.class);
		updatePredictorIntent.putExtra(EXTRA_UPDATE_PREDICTOR, true);
		updatePredictorPendingIntent = PendingIntent.getService(context, 0,
				updatePredictorIntent, 0);

		alarmsIntent = new Intent(context, AlarmsActivity.class);

		kgUnlockedNotification = new Notification(R.drawable.kg_unlocked, null,
				0);
		kgUnlockedNotification.flags |= Notification.FLAG_ONGOING_EVENT
				| Notification.FLAG_NO_CLEAR;

		kgUnlockedNotification.setLatestEventInfo(context,
				"Lock Screen Disabled", "Press to re-enable",
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		if (intent != null
				&& intent.getBooleanExtra(EXTRA_UPDATE_PREDICTOR, false))
			update(null);
	...
	public class MessageHandler extends Handler {
		...
		public void handleMessage(Message incoming) {
			...
			case RemoteConnection.SERVICE_CLIENT_CONNECTED:
				sendClientMessage(incoming.replyTo,
						RemoteConnection.CLIENT_SERVICE_CONNECTED);
				break;
	...
	/*
	 * public Boolean pluginHasSettings() { if (pluginServiceConnection.service
	 * == null) return false;
	 * 
	 * try { Class<?> c = pluginServiceConnection.service.getClass();
	 * java.lang.reflect.Method m = c.getMethod("hasSettings",
	 * EMPTY_CLASS_ARRAY); return (Boolean)
	 * m.invoke(pluginServiceConnection.service, EMPTY_OBJECT_ARRAY); } catch
	 * (Exception e) { e.printStackTrace(); return false; } }
	 * 
	 * public void configurePlugin() { if (pluginServiceConnection.service ==
	 * null) return;
	 * 
	 * try { Class<?> c = pluginServiceConnection.service.getClass();
	...
	private void update(Intent intent) {
		...
		syncSpsEditor(); // Important to sync after other Service code that uses
							// 'lasts' but before sending info to client

		for (Messenger messenger : clientMessengers) {
			// TODO: Can I send the same message to multiple clients instead of
			// sending duplicates?
			sendClientMessage(messenger,
					RemoteConnection.CLIENT_BATTERY_INFO_UPDATED,
					info.toBundle());
		}

		alarmManager.set(AlarmManager.ELAPSED_REALTIME,
				android.os.SystemClock.elapsedRealtime() + (2 * 60 * 1000),
				updatePredictorPendingIntent);
	}
	...
	private void prepareNotification() {
		...
		// TODO: Is it necessary to call new() every time here, or can I get
		// away with just setting the icon on existing Notif.?
		mainNotification = new Notification(iconFor(info.percent), null, 0l);

		mainNotification.flags |= Notification.FLAG_ONGOING_EVENT
				| Notification.FLAG_NO_CLEAR;

		if (settings.getBoolean(
				SettingsActivity.KEY_USE_SYSTEM_NOTIFICATION_LAYOUT, false)) {
			mainNotification.setLatestEventInfo(context,
					mainNotificationTopLine, mainNotificationBottomLine,
					mainWindowPendingIntent);
		} else {
			String icon_area = settings.getString(
					SettingsActivity.KEY_ICON_AREA,
			...
			notificationRV.setTextViewText(R.id.percent, "" + info.percent
					+ str.percent_symbol);
			notificationRV.setTextViewText(R.id.top_line,
					android.text.Html.fromHtml(mainNotificationTopLine));
			notificationRV.setTextViewText(R.id.bottom_line,
					mainNotificationBottomLine);

			int color;
			color = colorFor(
					SettingsActivity.KEY_NOTIFICATION_PERCENTAGE_TEXT_COLOR,
			...
			color = colorFor(
					SettingsActivity.KEY_NOTIFICATION_BOTTOM_LINE_COLOR,
					SettingsActivity.KEY_CUSTOM_BOTTOM_LINE_COLOR);
			if (color != 0)
				notificationRV.setTextColor(R.id.bottom_line, color);

			boolean default_show_box = res
					.getBoolean(R.bool.default_show_box_around_icon_area);
			boolean show_box = settings.getBoolean(
					SettingsActivity.KEY_SHOW_BOX_AROUND_ICON_AREA,
					default_show_box);

			if (show_box) {
				color = res.getColor(R.color.notification_box_default_color);
				if (!icon_area.equals("battery_first"))
					notificationRV.setInt(R.id.percent, "setBackgroundColor",
							color);
				if (!icon_area.equals("percentage_first"))
					notificationRV.setInt(R.id.battery, "setBackgroundColor",
							color);
			}
	...
	private int colorFor(String colorKey, String customKey) {
		...
		if (colorString.charAt(0) == '#')
			...
			return settings.getInt(customKey,
	...
	private String lineFor(String key) {
		String req = settings
				.getString(key,
						key.equals(SettingsActivity.KEY_TOP_LINE) ? "remaining"
								: "vitals");
	...
	private String predictionLine() {
		...
		if (info.prediction.what == BatteryInfo.Prediction.NONE) {
			...
			if (predicted.days > 0)
				...
				String verbosity = settings
						.getString(
								SettingsActivity.KEY_TIME_REMAINING_VERBOSITY,
								res.getString(R.string.default_time_remaining_verbosity));
				if (verbosity.equals("condensed"))
					line = str.n_hours_m_minutes_medium(predicted.hours,
							predicted.minutes);
				else if (verbosity.equals("verbose"))
					line = str.n_hours_m_minutes_long(predicted.hours,
							predicted.minutes);
				else
					line = str.n_hours_long_m_minutes_medium(predicted.hours,
							predicted.minutes);
			} else
	...
	private String vitalStatsLine() {
		Boolean convertF = settings.getBoolean(SettingsActivity.KEY_CONVERT_F,
				false);
		String line = str.healths[info.health] + " / "
				+ str.formatTemp(info.temperature, convertF);

		if (info.voltage > 500)
			line += " / " + str.formatVoltage(info.voltage);
		if (settings.getBoolean(
				SettingsActivity.KEY_STATUS_DURATION_IN_VITAL_SIGNS, false)) {
			float statusDurationHours = (now - info.last_status_cTM)
					/ (60 * 60 * 1000f);
			line += " / " + String.format("%.1f", statusDurationHours) + "h"; // TODO:
																				// Translatable
																				// 'h'
		}
	...
	private String statusDurationLine() {
		...
		if (statusDuration < 1000 * 60 * 60)
			line += str.since + " "
					+ formatTime(new Date(info.last_status_cTM));
		else
	...
	// I take advantage of (count on) R.java having resources alphabetical and
	// incrementing by one.
	private int iconFor(int percent) {
		...
		String icon_set = settings.getString(SettingsActivity.KEY_ICON_SET,
				"null");
		if (!icon_set.startsWith("builtin."))
			icon_set = "null"; // TODO: Remove this line to re-enable plugins

		if (icon_set.equals("null")) {
			icon_set = default_set;

			SharedPreferences.Editor settings_editor = settings.edit();
			settings_editor.putString(SettingsActivity.KEY_ICON_SET,
					default_set);
			settings_editor.commit();
		}

		Boolean indicate_charging = settings.getBoolean(
				SettingsActivity.KEY_INDICATE_CHARGING, true);

		if (icon_set.equals("builtin.plain_number")) {
			return ((info.status == BatteryInfo.STATUS_CHARGING && indicate_charging) ? chargingIcon0
					: plainIcon0)
					+ info.percent;
		} else if (icon_set.equals("builtin.smaller_number")) {
			return ((info.status == BatteryInfo.STATUS_CHARGING && indicate_charging) ? small_chargingIcon0
					: small_plainIcon0)
					+ info.percent;
		} else {
			if (settings.getBoolean(SettingsActivity.KEY_RED,
					res.getBoolean(R.bool.default_use_red))
					&& info.percent < Integer.valueOf(settings.getString(
							SettingsActivity.KEY_RED_THRESH,
							str.default_red_thresh))
					&& info.percent <= SettingsActivity.RED_ICON_MAX) {
				return R.drawable.r000 + info.percent - 0;
			} else if (settings.getBoolean(SettingsActivity.KEY_AMBER,
					res.getBoolean(R.bool.default_use_amber))
					&& info.percent < Integer.valueOf(settings.getString(
							SettingsActivity.KEY_AMBER_THRESH,
							str.default_amber_thresh))
			...
			} else if (settings.getBoolean(SettingsActivity.KEY_GREEN,
					res.getBoolean(R.bool.default_use_green))
					&& info.percent >= Integer.valueOf(settings.getString(
							SettingsActivity.KEY_GREEN_THRESH,
							str.default_green_thresh))
	...
	private boolean statusHasChanged() {
		...
		return (info.last_status != info.status
				...
				|| info.last_status_cTM > now
				|| info.last_plugged != info.plugged || (info.plugged == BatteryInfo.PLUGGED_UNPLUGGED && info.percent > previous_charge + 20));
	}
	...
	private void handleUpdateWithChangedStatus() {
		if (settings.getBoolean(SettingsActivity.KEY_ENABLE_LOGGING, true)) {
			...
			if (info.status != info.last_status
					&& info.last_status == BatteryInfo.STATUS_UNPLUGGED)
				log_db.prune(Integer.valueOf(settings.getString(
						SettingsActivity.KEY_MAX_LOG_AGE,
						str.default_max_log_age)));
		}

		/*
		 * TODO: Af first glance, I think I want to do this, but think about it
		 * a bit and decide for sure...
		 */
		if (info.status != info.last_status
				&& info.status == BatteryInfo.STATUS_UNPLUGGED)
			mNotificationManager.cancel(NOTIFICATION_ALARM_CHARGE);

		if (info.last_status != info.status
				&& settings.getBoolean(
						SettingsActivity.KEY_AUTO_DISABLE_LOCKING, false)) {
			if (info.last_status == BatteryInfo.STATUS_UNPLUGGED) {
				...
				/*
				 * If the screen was on, "inside" the keyguard, when the
				 * keyguard was disabled, then we're still inside it now, even
				 * if the screen is off. So we aquire a wakelock that forces the
				 * screen to turn on, then release it. If the screen is on now,
				 * this has no effect, but if it's off, then either the user
				 * will press the power button or the screen will turn itself
				 * off after the normal timeout. Either way, when the screen
				 * goes off, the keyguard will now be enabled properly.
				 */
	...
	private void handleAlarms() {
		...
		if (info.status == BatteryInfo.STATUS_FULLY_CHARGED
				&& info.status != info.last_status) {
			c = alarms.activeAlarmFull();
			if (c != null) {
				notification = parseAlarmCursor(c);
				notification.setLatestEventInfo(context,
						str.alarm_fully_charged, str.alarm_text, contentIntent);
				mNotificationManager.notify(NOTIFICATION_ALARM_CHARGE,
						notification);
				c.close();
			}
		}
		...
		if (c != null) {
			...
			notification
					.setLatestEventInfo(
							context,
							str.alarm_charge_drops
									+ c.getInt(alarms.INDEX_THRESHOLD)
									+ str.percent_symbol, str.alarm_text,
							contentIntent);
			mNotificationManager
					.notify(NOTIFICATION_ALARM_CHARGE, notification);
			c.close();
		}
		...
		c = alarms.activeAlarmTempRises(info.temperature,
				sp_store.getInt(KEY_PREVIOUS_TEMP, 1));
		if (c != null) {
			Boolean convertF = settings.getBoolean(
					SettingsActivity.KEY_CONVERT_F, false);
			sps_editor.putInt(KEY_PREVIOUS_TEMP, info.temperature);
			notification = parseAlarmCursor(c);
			notification.setLatestEventInfo(
					context,
					str.alarm_temp_rises
							+ str.formatTemp(c.getInt(alarms.INDEX_THRESHOLD),
									convertF, false), str.alarm_text,
					contentIntent);
			mNotificationManager.notify(NOTIFICATION_ALARM_TEMP, notification);
			c.close();
		}

		if (info.health > BatteryInfo.HEALTH_GOOD
				&& info.health != sp_store.getInt(KEY_PREVIOUS_HEALTH,
			...
			if (c != null) {
				...
				notification = parseAlarmCursor(c);
				notification.setLatestEventInfo(context,
						str.alarm_health_failure + str.healths[info.health],
						str.alarm_text, contentIntent);
				mNotificationManager.notify(NOTIFICATION_ALARM_HEALTH,
						notification);
				c.close();
			}
		}
	...
	private Notification parseAlarmCursor(Cursor c) {
		Notification notification = new Notification(
				R.drawable.stat_notify_alarm, null, now);
		...
		if (c.getInt(alarms.INDEX_VIBRATE) == 1)
			if (mAudioManager.getRingerMode() != mAudioManager.RINGER_MODE_SILENT)
				/*
				 * I couldn't get the Notification to vibrate, so I do it
				 * myself...
				 */
	...
	private void updateKeyguardNotification() {
		if (kl != null
				&& settings.getBoolean(
						SettingsActivity.KEY_NOTIFY_WHEN_KG_DISABLED, true))
			mNotificationManager.notify(NOTIFICATION_KG_UNLOCKED,
					kgUnlockedNotification);
		else
	...
	private void setupPlugins() {
		...
		String desiredPluginPackage = settings.getString(
				SettingsActivity.KEY_ICON_PLUGIN, "none");
		if (!desiredPluginPackage.equals("none")) {
			SharedPreferences.Editor settings_editor = settings.edit();
			settings_editor.putString(SettingsActivity.KEY_ICON_SET,
					desiredPluginPackage);
			settings_editor.putString(SettingsActivity.KEY_ICON_PLUGIN, "none");
			settings_editor.commit();
		}

		desiredPluginPackage = settings.getString(
				SettingsActivity.KEY_ICON_SET, "none");
		if (desiredPluginPackage.startsWith("builtin."))
			desiredPluginPackage = "none";

		if (!pluginPackage.equals(desiredPluginPackage)
				&& !pluginPackage.equals("none"))
			disconnectPlugin();

		if (!pluginPackage.equals(desiredPluginPackage)
				&& !desiredPluginPackage.equals("none")) {
			try {
				Context pluginContext = getApplicationContext()
						.createPackageContext(desiredPluginPackage,
								Context.CONTEXT_INCLUDE_CODE);
				ClassLoader pluginClassLoader = pluginContext.getClassLoader();
				Class pluginClass = pluginClassLoader
						.loadClass(desiredPluginPackage + ".PluginService");
