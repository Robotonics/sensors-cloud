...
public class LogDatabase {
	...
	public Cursor getAllLogs(Boolean reversed) {
		...
		return rdb.rawQuery("SELECT * FROM " + LOG_TABLE_NAME + " ORDER BY "
				+ KEY_TIME + " " + order, null);
	}
	...
	public void logStatus(BatteryInfo info, long time, int status_age) {
		...
		if (lastLog.moveToFirst()) {
			int statusCode = lastLog.getInt(lastLog
					.getColumnIndexOrThrow(KEY_STATUS_CODE));
			int lastCharge = lastLog.getInt(lastLog
					.getColumnIndexOrThrow(KEY_CHARGE));
			int[] a = decodeStatus(statusCode);
			int lastStatus = a[0];
			int lastPlugged = a[1];

			if (info.percent == lastCharge && info.status == lastStatus
					&& info.plugged == lastPlugged)
				duplicate = true;
		}

		if (!duplicate)
			wdb.execSQL("INSERT INTO " + LOG_TABLE_NAME + " VALUES (NULL, "
					+ encodeStatus(info.status, info.plugged, status_age)
					+ " ," + info.percent + " ," + time + " ,"
	...
	public void prune(int max_hours) {
		...
		wdb.execSQL("DELETE FROM " + LOG_TABLE_NAME + " WHERE " + KEY_TIME
				+ " < " + oldest_log);
	}

	/*
	 * My cursor adapter was getting a bit complicated since it could only see
	 * one datum at a time, and how I want to present the data depends on
	 * several interrelated factors. Storing all three of these items together
	 * simplifies things.
	 */
	...
	private static class SQLOpenHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + LOG_TABLE_NAME + " (" + KEY_ID
					+ " INTEGER PRIMARY KEY," + KEY_STATUS_CODE + " INTEGER,"
					+ KEY_CHARGE + " INTEGER," + KEY_TIME + " INTEGER,"
					+ KEY_TEMPERATURE + " INTEGER," + KEY_VOLTAGE + " INTEGER"
					+ ");");
		}
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			if (oldVersion == 3 && newVersion == 4) {
				db.execSQL("ALTER TABLE " + LOG_TABLE_NAME + " ADD COLUMN "
