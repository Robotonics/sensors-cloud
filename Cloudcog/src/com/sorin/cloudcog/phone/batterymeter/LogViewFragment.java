...
public class LogViewFragment extends ListFragment {
	...
	private static final String[] CSV_ORDER = { LogDatabase.KEY_TIME,
			LogDatabase.KEY_STATUS_CODE, LogDatabase.KEY_CHARGE,
	...
	private final BroadcastReceiver mBatteryInfoReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			// TODO: Do this better. Perphaps have the Activity be the client of
			// the Service, or perhaps
			// leave it with CIF as client, but it calls Activity.infoUpdated()
			// after receiving message
			// from Service. If so, make sure (and document) that Service
			// shouldn't send that message
			// until after logs have been updated (and not just info). Perhaps
			// even renaming message.
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View logs_header = View.inflate(activity.context, R.layout.logs_header,
				null);
	...
	public void onResume() {
		super.onResume();
		activity.context.registerReceiver(mBatteryInfoReceiver,
	...
	public class ConfirmClearLogsDialogFragment extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			return new AlertDialog.Builder(activity)
					.setTitle(
							activity.res.getString(R.string.confirm_clear_logs))
	...
	public class ConfigureLogFilterDialogFragment extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			for (int i = 0; i < checked_items.length; i++) {
				checked_items[i] = activity.settings.getBoolean(
						activity.str.log_filter_pref_keys[i], true);
			}

			return new AlertDialog.Builder(activity)
					.setTitle(
							activity.res
									.getString(R.string.configure_log_filter))
					...
					.setPositiveButton(activity.res.getString(R.string.okay),
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface di, int id) {
									di.cancel(); // setFilters() is called in
													// onCancel()
								}
							}).create();
		}
	...
	private void setFilters(boolean[] checked_items) {
		...
		for (int i = 0; i < checked_items.length; i++) {
			editor.putBoolean(activity.str.log_filter_pref_keys[i],
					checked_items[i]);
		}
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		inflater.inflate(R.menu.phone_battery_logs, menu);
	}
	...
	private void exportCSV() {
		...
		if (state != null && state.equals(Environment.MEDIA_MOUNTED_READ_ONLY)) {
			Toast.makeText(activity.context, activity.str.read_only_storage,
					Toast.LENGTH_SHORT).show();
			return;
		} else if (state == null || !state.equals(Environment.MEDIA_MOUNTED)) {
			Toast.makeText(activity.context,
					activity.str.inaccessible_w_reason + state,
					Toast.LENGTH_SHORT).show();
			return;
		}
		...
		String[] csvFields = { activity.str.date, activity.str.time,
				activity.str.status, activity.str.charge,
				activity.str.temperature, activity.str.temperature_f,
				activity.str.voltage };

		try {
			if (!csvFile.createNewFile() || !csvFile.canWrite()) {
				Toast.makeText(activity.context,
						activity.str.inaccessible_storage, Toast.LENGTH_SHORT)
						.show();
				return;
			}
			...
			for (completeCursor.moveToFirst(); !completeCursor.isAfterLast(); completeCursor
					.moveToNext()) {
				cols = CSV_ORDER.length;
				for (i = 0; i < cols; i++) {
					if (CSV_ORDER[i].equals(LogDatabase.KEY_TIME)) {
						d.setTime(completeCursor.getLong(mAdapter.timeIndex));
						buf.write(mAdapter.dateFormat.format(d) + ","
								+ mAdapter.timeFormat.format(d) + ",");
					} else if (CSV_ORDER[i].equals(LogDatabase.KEY_STATUS_CODE)) {
						statusCode = completeCursor
								.getInt(mAdapter.statusCodeIndex);
						...
						buf.write(String.valueOf(completeCursor
								.getInt(mAdapter.chargeIndex)) + ",");
					} else if (CSV_ORDER[i].equals(LogDatabase.KEY_TEMPERATURE)) {
						int temperature = completeCursor
								.getInt(mAdapter.temperatureIndex);
						buf.write(String.valueOf(temperature / 10.0) + ",");
						buf.write(String.valueOf(java.lang.Math
								.round(temperature * 9 / 5.0) / 10.0 + 32.0)
								+ ",");
					} else if (CSV_ORDER[i].equals(LogDatabase.KEY_VOLTAGE)) {
						buf.write(String.valueOf(completeCursor
								.getInt(mAdapter.voltageIndex) / 1000.0));
					}
				}
				buf.write("\r\n");
			}
			...
			Toast.makeText(activity.context, activity.str.inaccessible_storage,
					Toast.LENGTH_SHORT).show();
			return;
		}

		Toast.makeText(activity.context, activity.str.file_written,
				Toast.LENGTH_SHORT).show();
	}
	...
	private class FilteredCursor extends CursorWrapper {
		...
		public void refilter() {
			...
			int statusCodeIndex = wrappedCursor
					.getColumnIndexOrThrow(LogDatabase.KEY_STATUS_CODE);

			boolean show_plugged_in = activity.settings.getBoolean(
					"plugged_in", true);
			boolean show_unplugged = activity.settings.getBoolean("unplugged",
					true);
			boolean show_charging = activity.settings.getBoolean("charging",
					true);
			boolean show_discharging = activity.settings.getBoolean(
					"discharging", true);
			boolean show_fully_charged = activity.settings.getBoolean(
					"fully_charged", true);
			boolean show_unknown = activity.settings
					.getBoolean("unknown", true);

			for (wrappedCursor.moveToFirst(); !wrappedCursor.isAfterLast(); wrappedCursor
					.moveToNext()) {
				...
				if (status == BatteryInfo.STATUS_FULLY_CHARGED
					...
					shownIDs.add(wrappedCursor.getPosition());
						...
						|| status == BatteryInfo.STATUS_NOT_CHARGING || status > BatteryInfo.STATUS_MAX)
						&& show_unknown) {
					shownIDs.add(wrappedCursor.getPosition());
	...
	private class TimeDeltaCursor extends CursorWrapper {
		...
		private void gen_delta() {
			...
			if (status == BatteryInfo.STATUS_FULLY_CHARGED) {
				if (last_plugged > 0)
					...
					&& status == BatteryInfo.STATUS_UNPLUGGED) {
				if (last_plugged > 0)
					deltas.add(time - last_plugged);
				else
					deltas.add(-1l);
		...
		public void gen_deltas() {
			...
			int wrappedCursorPos = wrappedCursor.getPosition();
			statusCodeIndex = wrappedCursor
					.getColumnIndexOrThrow(LogDatabase.KEY_STATUS_CODE);
			timeIndex = wrappedCursor
					.getColumnIndexOrThrow(LogDatabase.KEY_TIME);
			...
			if (wrappedIsChronological())
				...
				for (wrappedCursor.moveToLast(); !wrappedCursor.isBeforeFirst(); wrappedCursor
						.moveToPrevious())
					gen_delta();
		...
		// TODO: This was introduced in API 11. It seems to be safely ignored by
		// APIs less than 11, other than
		// taking a while for the verifier to no-op it. Is it worth overriding
		// to add only to
		// API 11+ devices? I think it only slows things down on the first run,
		// and not noticeably on most devices.
		/*
		 * @Override public int getType(int columnIndex) { if (columnIndex ==
		 * deltaColumnIndex) return Cursor.FIELD_TYPE_INTEGER; else return
		 * super.getType(columnIndex); }
		 */
	...
	private class LogAdapter extends CursorAdapter {
		public int statusCodeIndex, chargeIndex, timeIndex, temperatureIndex,
				voltageIndex, timeDeltaIndex;
		...
		public LogAdapter(Context context, Cursor cursor) {
			...
			temperatureIndex = cursor
					.getColumnIndexOrThrow(LogDatabase.KEY_TEMPERATURE);
			voltageIndex = cursor
					.getColumnIndexOrThrow(LogDatabase.KEY_VOLTAGE);
			timeDeltaIndex = cursor
					.getColumnIndexOrThrow(TimeDeltaCursor.KEY_TIME_DELTA);
		}
		...
		public void bindView(View view, Context context, Cursor cursor) {
			...
			TextView temp_volt_tv = (TextView) view
					.findViewById(R.id.temp_volt);
			TextView time_diff_tv = (TextView) view
			...
			if (status_age == LogDatabase.STATUS_OLD) {
				...
				case 5:
					...
					time_diff_tv.setText(String.format(activity.res
							.getString(R.string.after_n_hours_plugged_in),
				...
				case 2:
					...
					time_diff_tv.setText(String.format(activity.res
							.getString(R.string.after_n_hours_unplugged),
							delta / 1000.0 / 60.0 / 60.0));
			...
			if (temperature != 0)
				temp_volt_tv.setText(""
						+ activity.str.formatTemp(temperature, convertF));
			else
				temp_volt_tv.setText(""); /* TextViews are reused */

			int voltage = cursor.getInt(voltageIndex);
			if (voltage != 0)
				temp_volt_tv.setText(((String) temp_volt_tv.getText()) + " / "
