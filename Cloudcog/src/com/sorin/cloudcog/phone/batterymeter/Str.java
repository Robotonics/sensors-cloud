...
public class Str {
	...
	public Str(Resources r) {
		...
		default_main_notification_priority = res
				.getString(R.string.default_main_notification_priority);
		...
		confirm_ten_percent_enable = res
				.getString(R.string.confirm_ten_percent_enable);
		confirm_ten_percent_disable = res
				.getString(R.string.confirm_ten_percent_disable);
		confirm_ten_percent_hint = res
				.getString(R.string.confirm_ten_percent_hint);
	...
	public String for_n_hours(int n) {
		return String
				.format(res.getQuantityString(R.plurals.for_n_hours, n), n);
	}

	public String n_hours_m_minutes_long(int n, int m) {
		return (String.format(res.getQuantityString(R.plurals.n_hours_long, n),
				n) + String.format(
				res.getQuantityString(R.plurals.n_minutes_long, m), m));
	}

	public String n_minutes_long(int n) {
		return String.format(
				res.getQuantityString(R.plurals.n_minutes_long, n), n);
	}

	public String n_hours_m_minutes_medium(int n, int m) {
		return (String.format(
				res.getQuantityString(R.plurals.n_hours_medium, n), n) + String
				.format(res.getQuantityString(R.plurals.n_minutes_medium, m), m));
	}

	public String n_hours_long_m_minutes_medium(int n, int m) {
		return (String.format(res.getQuantityString(R.plurals.n_hours_long, n),
				n) + String.format(
				res.getQuantityString(R.plurals.n_minutes_medium, m), m));
	}

	public String n_hours_m_minutes_short(int n, int m) {
		return (String.format(
				res.getQuantityString(R.plurals.n_hours_short, n), n) + String
				.format(res.getQuantityString(R.plurals.n_minutes_short, m), m));
	}

	public String n_days_m_hours(int n, int m) {
		return (String.format(res.getQuantityString(R.plurals.n_days, n), n) + String
				.format(res.getQuantityString(R.plurals.n_hours, m), m));
	}

	public String n_log_items(int n) {
		return String
				.format(res.getQuantityString(R.plurals.n_log_items, n), n);
	}

	/*
	 * temperature is the integer number of tenths of degrees Celcius, as
	 * returned by BatteryManager
	 */
	public String formatTemp(int temperature, boolean convertF,
			boolean includeTenths) {
		double d;
		String s;
		...
		// Weird: the ternary operator seems to compile down to a "function"
		// that has to return a single particular type
		// return "" + (includeTenths ? d : java.lang.Math.round(d)) + s;
		return (includeTenths ? String.valueOf(d) : String
				.valueOf(java.lang.Math.round(d))) + s;
	}
	...
	public static void overrideLanguage(Resources res, WindowManager wm,
		...
		if (!lang_override.equals("default")) {
			...
			/*
			 * Perhaps showing a confirmation dialog, saying the app needs to
			 * close in order for change to take effect. You'd actually do that
			 * from SettingsActivity, so execution would never actually get
			 * here.
			 */
		}
	}
