...
public final class Gauge extends View implements SensorEventListener {
	...
	public int incrementPerLargeNotch = 10;
	public int incrementPerSmallNotch = 2;
	private float degreesPerNotch = 360.0f / totalNotches;

	public int scaleCenterValue = 0; // the one in the top center (12 o'clock)
	public int scaleMinValue = 0;
	public int scaleMaxValue = 130;
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (context != null && attrs != null) {
			TypedArray a = context.obtainStyledAttributes(attrs,
					R.styleable.Gauge);
			// totalNotches = a.getInt(R.styleable.Gauge_totalNotches,
			// totalNotches);
			// incrementPerLargeNotch =
			// a.getInt(R.styleable.Gauge_incrementPerLargeNotch,
			// incrementPerLargeNotch);
			// incrementPerSmallNotch =
			// a.getInt(R.styleable.Gauge_incrementPerSmallNotch,
			// incrementPerSmallNotch);
			// scaleCenterValue = a.getInt(R.styleable.Gauge_scaleCenterValue,
			// scaleCenterValue);
			// scaleMinValue = a.getInt(R.styleable.Gauge_scaleMinValue,
			// scaleMinValue);
			// scaleMaxValue = a.getInt(R.styleable.Gauge_scaleMaxValue,
			// scaleMaxValue);
			scaleColor = a.getInt(R.styleable.Gauge_scaleColor, scaleColor);

			/*
			 * String scaleUpperTitle =
			 * a.getString(R.styleable.Gauge_scaleUpperTitle); String
			 * scaleLowerTitle = a.getString(R.styleable.Gauge_scaleLowerTitle);
			 * 
			 * if (scaleUpperTitle != null) this.scaleUpperTitle =
			 * scaleUpperTitle; if (scaleLowerTitle != null)
			 * this.scaleLowerTitle = scaleLowerTitle;
			 */
		}
		initDrawingTools();
	}
	...
	private void drawTitle(Canvas canvas) {
		// Use a vertical offset when printing the title in the upper half of
		// the scale.
		// The upper and lower half use the same rectangular but the spacing
		// between the
		// title and the scale is not equal for the upper and lower half of the
		// dial and
		// therefore you should move the upper title downwards.
		canvas.drawTextOnPath(scaleUpperTitle, titleUpperPath, 0.0f, 0.02f,
				titlePaint);

		// We print the title on the lower half of the scale.
		canvas.drawTextOnPath(scaleLowerTitle, titleLowerPath, 0.0f, 0.0f,
				titlePaint);
	}
	...
	private void drawLogo(Canvas canvas) {
		...
		/*
		 * int color = 0x00000000; float position =
		 * getRelativeTemperaturePosition(); if (position < 0) { color |= (int)
		 * ((0xf0) * -position); // blue } else { color |= ((int) ((0xf0) *
		 * position)) << 16; // red } //Log.d(TAG, "*** " +
		 * Integer.toHexString(color)); LightingColorFilter logoFilter = new
		 * LightingColorFilter(0xff338822, color);
	...
	private float getRelativeTemperaturePosition() {
		if (handPosition < scaleCenterValue) {
			return -(scaleCenterValue - handPosition)
					/ (float) (scaleCenterValue - scaleMinValue);
		} else {
			return (handPosition - scaleCenterValue)
					/ (float) (scaleMaxValue - scaleCenterValue);
		}
	}

	void setHandTarget(float temperature) {
