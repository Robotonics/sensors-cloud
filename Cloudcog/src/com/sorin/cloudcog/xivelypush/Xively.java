...
import com.sorin.cloudcog.xivelypush.httpclient.HttpClient;
import com.sorin.cloudcog.xivelypush.httpclient.HttpMethod;
import com.sorin.cloudcog.xivelypush.httpclient.HttpRequest;
import com.sorin.cloudcog.xivelypush.httpclient.HttpResponse;
...
public class Xively {
	 ...
	 * @param APIKEY
	 */
	public Xively(String APIKEY) {
		super();
		this.API_KEY = APIKEY;
		this.client = new HttpClient("www.cosm.com");
	}
	 ...
	 * @throws XivelyException
	 *             If something goes wrong.
	 */
	public Feed getFeed(int feed) throws XivelyException {
		...
		if (g.getHeaderItem("Status").equals("HTTP/1.1 200 OK")) {
			return XivelyFactory.toFeed(this, g.getBody());
		} else {
			throw new XivelyException(g.getHeaderItem("Status"));
		}
	}
	 ...
	 *             If something goes wrong.
	 */
	public Feed createFeed(Feed f) throws XivelyException {
	 ...
	 * @throws XivelyException
	 */
	public boolean updateFeed(int feed, String s) throws XivelyException {
		HttpRequest hr = new HttpRequest("http://api.cosm.com/v2/feeds/" + feed
				+ ".xml");
	 ...
	 * @throws XivelyException
	 */
	public boolean createDatastream(int feed, String s) throws XivelyException {
		HttpRequest hr = new HttpRequest("http://api.cosm.com/v2/feeds/" + feed
				+ "/datastreams/");
		hr.setMethod(HttpMethod.POST);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
	 ...
	 * @throws XivelyException
	 */
	public String createTrigger(Trigger t) throws XivelyException {
		...
		if (h.getHeaderItem("Status").equals("HTTP/1.1 201 Created")) {
			...
			throw new XivelyException(h.getHeaderItem("Status"));
		}
	 ...
	 */
	public Trigger getTrigger(int id) throws XivelyException {
		HttpRequest hr = new HttpRequest("http://api.cosm.com/v2/triggers/"
				+ id + ".xml");
		hr.setMethod(HttpMethod.GET);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
		...
		return XivelyFactory.toTrigger(h.getBody())[0];
	 ...
	 *            id of the Trigger to get
	 */
	public Trigger[] getTriggers() throws XivelyException {
		HttpRequest hr = new HttpRequest("http://api.cosm.com/v2/triggers/");
		hr.setMethod(HttpMethod.GET);
		hr.addHeaderItem("X-PachubeApiKey", this.API_KEY);
		HttpResponse h = client.send(hr);

		return XivelyFactory.toTrigger(h.getBody());
	 ...
	 * 
	 * @param id
	 *            id of the triggerto update
	 * @param t
	 *            Trigger object of the new trigger
	 * @return
	 */
	 ...
	 * Color c) { String hexRed = Integer.toHexString(c.getRed()).toString();
	 * String hexGreen = Integer.toHexString(c.getGreen()).toString(); String
	 * hexBlue = Integer.toHexString(c.getBlue()).toString(); if
	 * (hexRed.length() == 1) { hexRed = "0" + hexRed; }
	 * 
	 * if (hexGreen.length() == 1) { hexGreen = "0" + hexGreen; } if
	 * (hexBlue.length() == 1) { hexBlue = "0" + hexBlue; } String hex = (hexRed
	 * + hexGreen + hexBlue).toUpperCase();
	 * 
	 * return "http://www.pachube.com/feeds/" + feedID + "/datastreams/" +
	 * streamID + "/history.png?w=" + width + "&h=" + height + "&c=" + hex;
	 * 
	 * }
	 */
